...
public class GeocodeSearchService {
	...
	public GeocodeSearchService(WebClient.Builder webClientBuilder,
			@Value("${ors.geocodesearch.uri}") String geocodeSearchUri) {
	...
	public Mono<List<GeocodeSearchResponseDTO>> searchAddress(String text) {
		return webClient.get()
				.uri(uriBuilder -> uriBuilder.queryParam("api_key", apiKey).queryParam("text", text)
						.queryParam("boundary.country", "TR").queryParam("size", 3).build())
				.retrieve().bodyToMono(ORSGeocodeSearchResponseDTO.class)
				.map(response -> response.getFeatures().stream().map(feature -> {
