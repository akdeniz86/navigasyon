...
public class RouteService {
    ...
    public RouteService(WebClient.Builder webClientBuilder,
            TollWaysRepository tollPriceRepository) {
    			this.webClient = webClientBuilder.build();
    			this.tollWaysRepository = tollPriceRepository;
    	}
	

	public Mono<RouteResponseDTO> calculateRoute(RouteRequestDTO requestDTO) {
		return webClient.post().uri(directionsUri).header("Authorization", apiKey).bodyValue(requestDTO).retrieve()
				.bodyToMono(new ParameterizedTypeReference<Map<String, Object>>() {
					...
					return new RouteResponseDTO(distance, duration, allTollFees, instructions, geoCoord);
				});
	}
